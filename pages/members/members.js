...
Page({
  ...
  zhuanz(){
    wx.navigateTo({
      url: '/pages/card/assets_expel/assets_expel',
    })
  },
  //明细
  minx(){
   wx.navigateTo({
     url: '/pages/card/assets_mx/assets_mx',
   })
  },
  hide4() {
    this.setData({
  ...
  web(){
     wx.navigateTo({
       url: '../agreement_store/agreement_store?src=' + 'https://www.xingtu-group.cn/sjg_xieyi/6_gns_VIP.html',
     })
  },
  navBack(){
    if(userid){
  ...
  //购买会员
   pay(e){
     ...
     if(that.data.user.memberType == 0){
       app.res.req('/sqorder/membersubmit', data, (res) => {
         console.log(res.data)
         if (res.status == 1000) {
           wx.showLoading({
             mask: true
  ...
  Detail(){
    ...
    app.res.req('/memberparameter/info', data, (res) => {
      console.log(res.data)
      if (res.status == 1000) {
        wx.hideLoading()
        that.setData({
